...
I want to include `colour` as a predictor, so I'm going to stick it onto the end of the `description`.
```{r adding_some_colour, results='asis'}
wine <- wine %>% 
  mutate(
    description = case_when(
      colour == "red" ~ description, # defined by absence
      TRUE ~ paste0(description, " colour_", colour)
    )
  )
paste0("> ", wine$description[12412]) %>% cat
```

Let's prepare our train and test sets:
```{r wine_split}
...
We need a function that maps an **arbitrary string** (and wine colour) onto this vector space.
...
> "Tastes sour and bone dry, with grapefruit flavors. colour_white"

Prediction: `r round(predict(xgb, map_to_dtm(filter(wine, index == 78883)$description)), digits = 2)` points

--
> "Tastes and bone dry with grapefruit flavors. colour_white"

Prediction: `r round(predict(xgb, map_to_dtm("Tastes and bone dry with grapefruit flavors. colour_white")), digits = 2)` points

--
> "Tastes sour and bone dry, with grapefruit flavors."

Prediction: `r round(predict(xgb, map_to_dtm("Tastes sour and bone dry with grapefruit flavors")), digits = 2)` points
