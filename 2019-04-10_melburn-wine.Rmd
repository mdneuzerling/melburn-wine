...
We're about to do some modelling, so let's prepare our train and test sets.

```{r wine_split}
wine_split <- rsample::initial_split(wine, strata = "colour", p = 0.80)
...
```{r sample_wine_description, results='asis', echo = FALSE}
wine %>% 
  filter(index == 17640) %>% 
  pull(description) %>% 
  paste0("> ", .) %>% 
...
```{r wine_17640_explanation}
wine_17640 <- filter(wine, index == 17640)
wine_17640_explanation <- lime::explain(
  wine_17640$description,
  wine_explainer,
  n_features = 10
)
```

```{r wine_17640_explanation_plot, eval=FALSE}
custom_lime_explanation_plot(
  wine_17640_explanation, wine_17640
)
```
]

.pull-right[
```{r wine_17640_explanation_plot_out, ref.label="wine_17640_explanation_plot", echo=FALSE, dpi = 300, fig.width = 5, fig.height = 5, out.height = 500, out.width = 500}
...
```{r sample_wine_2_description, results='asis', echo = FALSE}
wine %>% 
  filter(index == 85) %>% 
  pull(description) %>% 
  paste0("> ", .) %>% 
...
```{r wine_85_explanation}
wine_85 <- filter(wine, index == 85)
wine_85_explanation <- lime::explain(
  wine_85$description,
  wine_explainer,
  n_features = 10
)
...
```{r wine_85_explanation_plot, eval=FALSE}
custom_lime_explanation_plot(
  wine_85_explanation, wine_85
)
```
]

.pull-right[
```{r wine_85_explanation_plot_out, ref.label="wine_85_explanation_plot", echo=FALSE, dpi = 300, fig.width = 5, fig.height = 5, out.height = 500, out.width = 500}
```
]